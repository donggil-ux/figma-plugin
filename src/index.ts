    ...
    tools: [
      {
        name: "get_file",
        description:
          "Figma 파일의 정보를 조회합니다. 파일 구조, 컴포넌트, 스타일 등의 메타데이터를 반환합니다.",
        inputSchema: {
          type: "object",
          properties: {
            fileKey: {
              type: "string",
              description:
                "Figma 파일 키 (URL에서 /file/ 뒤의 부분, 예: figma.com/file/ABC123/... 에서 ABC123)",
            },
            depth: {
              type: "number",
      ...
      },
      {
        name: "get_node",
        description:
          "Figma 파일 내 특정 노드의 상세 정보를 조회합니다.",
        inputSchema: {
          type: "object",
          properties: {
            fileKey: {
            ...
            nodeId: {
              type: "string",
              description:
                "노드 ID (URL의 node-id 파라미터, 예: 123:456 또는 123-456)",
            },
          },
      ...
      },
      {
        name: "get_styles",
        description:
          "Figma 파일에 정의된 스타일(색상, 텍스트, 효과 등)을 조회합니다.",
        inputSchema: {
          type: "object",
          properties: {
            fileKey: {
      ...
      },
      {
        name: "export_image",
        description:
          "Figma 노드를 이미지로 내보내기합니다. PNG, SVG, JPG, PDF 형식을 지원합니다.",
        inputSchema: {
          type: "object",
          properties: {
            fileKey: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "get_node": {
        const { fileKey, nodeId } = args as { fileKey: string; nodeId: string };
      ...
      case "export_image": {
        const { fileKey, nodeId, format, scale } = args as {
          fileKey: string;
          nodeId: string;
